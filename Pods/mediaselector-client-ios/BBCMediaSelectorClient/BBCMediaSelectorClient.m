...
- (void)sendMediaSelectorRequest:(BBCMediaSelectorRequest*)request success:(BBCMediaSelectorClientResponseBlock)success failure:(BBCMediaSelectorClientFailureBlock)failure
{
    ...
    [self prepareAuthenticationForRequest:requestWithMediaSet success:^(BBCMediaSelectorRequest * requestToSend) {
        ...
        if (strongSelf) {
            BBCMediaSelectorURLBuilder* urlBuilder = [[BBCMediaSelectorURLBuilder alloc] initWithConfiguring: strongSelf.configuring];
            NSString* url = [urlBuilder urlForRequest:requestToSend];
            
            BBCMediaSelectorRequestHeadersBuilder* requestHeadersBuilder = [[BBCMediaSelectorRequestHeadersBuilder alloc] initWithConfiguring:strongSelf.configuring];
            NSDictionary* headers = [requestHeadersBuilder headersForRequest:requestToSend];
            
            [strongSelf sendRequestForURL:url withHeaders:headers withSecureConnectionPreference:requestToSend.secureConnectionPreference success:success failure:failure];
        }
